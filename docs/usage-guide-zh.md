...
查询 Kiro（AWS AI-powered IDE）的功能、配置、Specs、Hooks、Steering 和 MCP 等相关文档。

**示例问题：**
- `@TechBot Kiro 的 hooks 怎么配置？`
- `@TechBot Kiro 支持哪些 MCP server？`

### 6. AWS 账号操作

TechBot 可以查询、创建和修改用户 AWS 账号中的资源。只读操作（查看资源、查询费用）会直接执行，写操作（创建资源、修改配置）会先给出 CLI 命令让用户确认后再执行。

**示例问题：**
- `@TechBot 帮我看看 us-west-2 有哪些 Lambda 函数`
- `@TechBot 帮我查一下本月 AWS 总花费`
- `@TechBot 帮我检查一下我的 S3 bucket 有没有公开访问的安全隐患`
- `@TechBot 帮我给 techbot-worker 创建一个 CloudWatch 告警`
- `@TechBot 帮我看看 techbot-worker 最近有没有报错`
...
- **建议在提问中指明区域和时间范围**（如"us-west-2 的 Lambda"、"2026年6月的花费"），这能减少不必要的检索，节省 token 消耗
- 价格数据为实时查询，以 AWS 官方定价为准
- 中国区和全球区的服务差异较大，请在提问时明确指定区域
- 回复末尾会显示本次 token 消耗和预估成本，以及参考链接
- 如果问题比较复杂（涉及多个服务或多步操作），建议拆分为多个子问题，通过多轮对话逐步完成。例如先问"帮我查一下本月花费最多的服务"，再追问"帮我给花费最高的服务设一个 budget 告警"
- 操作类问题（涉及 AWS 资源操作）的响应时间通常在 30s-240s，执行过程中飞书卡片会实时显示当前进度。TechBot 内置了超时保护：240s 后会引导模型用已有信息给出部分回答，300s 后会直接返回超时提示
- 若 Agent 回答失败，可以尝试换一种更清晰的提问方式。若没有使用 GLM-5 模型，也可以选择该模型，性能最好
