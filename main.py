...
async def invoke(payload):
    ...
    with client:
        ...
        session_manager = _setup_session_manager(session_id, actor_id)
        ...
        _register_hooks(agent, payload)

        healthy_status.value = "HealthyBusy"
        logger.info(f"🚀 Agent job starts | actor={actor_id} session={session_id}")

        final_text = await _execute_agent(agent, user_text, payload)
        final_text = _append_token_usage(agent, model_id, user_text, final_text)
...
def _setup_session_manager(session_id: str, actor_id: str):
...
def _register_hooks(agent, payload):
    """Register all runtime guard hooks on the agent.

    Hooks share per-request state via closures, so they are defined here
    (one set per invocation) rather than at module level.
    """
    from strands.hooks import BeforeToolCallEvent, BeforeModelCallEvent, AfterToolCallEvent
...
async def _execute_agent(agent, user_text: str, payload) -> str:
    """Run the agent (text or text+image) with a hard timeout; return final text."""
    import asyncio
...
def _append_token_usage(agent, model_id: str, user_text: str, final_text: str) -> str:
    """Log token usage and append cost info to the response (match user language)."""
    MODEL_PRICING = {
